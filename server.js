...
let config = {
  granularity: 60,        // timeframe en segundos (M1)
  stake: 1,               // monto por operación en USD
  duration: 1,            // duración en minutos
  riskPercentage: 1,      // % del balance a arriesgar (no implementado aún, por si quieres)
  maxOperationsPerHour: 3 // límite para evitar overtrading
};
...
function ejecutarOperacion(decision) {
  if (operacionesUltimaHora >= config.maxOperationsPerHour) {
    console.log('⛔ Límite de operaciones por hora alcanzado.');
    return;
  }
  ...
  ws.on('error', err => {
    console.error('❌ Error operando:', err.message);
  });
}
